...
def hook(hook_api):
    ...
    for root, _, __ in os.walk(plugin_src_dir):
        pattern_path = join(root, DYLIB_PATTERN)
        plugin_src_files.extend(glob(pattern_path))
    ...
    for f in plugin_src_files:
        ...
        dst_rel_dir = join(dst_root_name, "plugins", relpath(f, src_plugins_root))
        bin_tuple = (f, dst_rel_dir)
